...
exports.createFolder = async (req, res) => {
  console.log("folder creation req received");
  console.log(req.body);
  const { Foldername, CreatedDate } = req.body;
  console.log(Foldername, CreatedDate);
  ...
  try {
    ...
    newFolder.save()
    res.status(200).json({ "new folder created with name": Foldername });
  } catch (err) {
    console.error("Error craeting folder: ", err);
    res.status(500).send(err.message);
  }
};
...
exports.getFolders = async (req, res)=>{
  console.log("folder list request recived ");
  try{
